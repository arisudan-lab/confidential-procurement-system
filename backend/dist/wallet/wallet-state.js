...
export const CHILD_KINDS = ['shielded', 'unshielded', 'dust'];
function networkDir(network, opts = {}) {
    return path.join(opts.cwd ?? process.cwd(), WALLET_STATE_DIR, network);
}
function statePath(network, kind, opts = {}) {
...
function readVersionedState(file) {
...
function writeVersionedState(file, state) {
    const payload = { version: WALLET_STATE_VERSION, state };
    atomicWrite(file, `${JSON.stringify(payload)}\n`);
}
export function loadWalletState(network, opts = {}) {
    return {
        ...
        dust: readVersionedState(statePath(network, 'dust', opts)),
    };
}
export function saveWalletState(network, state, opts = {}) {
    ...
    if (state.unshielded !== undefined)
        writeVersionedState(statePath(network, 'unshielded', opts), state.unshielded);
    if (state.dust !== undefined)
        writeVersionedState(statePath(network, 'dust', opts), state.dust);
}
export function clearWalletState(network, opts = {}) {
